...
int main(int argc, char* argv[])
{
	...
	bool tts_flag = false;
	ros::init(argc, argv, "yolo_tts_node");    //初始化节点，节点名为"castle_xf_tts_node"
	ros::NodeHandle n;
	// yolo物体识别并播放
	ros::Subscriber sub_label = n.subscribe("/yolo_label_topic", 3, YoloCallBack);    //	订阅语义理解话题
	ros::Subscriber sub_asr = n.subscribe("yolo_asr_topic", 1, AsrCallBack);    //	订阅语义理解话题

	asr_pub = n.advertise<std_msgs::Int32>("arv_awake_topic", 1);  //	发布语音唤醒数据话题
	ros::NodeHandle nh("~");    //用于launch文件传递参数
	nh.param("appid", appid, string("appid = 604adfa0, work_dir = ."));    //从launch文件获取参数
