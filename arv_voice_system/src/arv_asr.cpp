...
int main(int argc, char** argv)
{
	...
	ros::init(argc, argv, "arv_asr_node");    //初始化节点，向节点管理器注册
	ros::NodeHandle n;
	ros::Subscriber sub = n.subscribe("arv_awake_topic", 1, asrCallback);	//	订阅语音唤醒话题
	//ros::Subscriber sub_tts = n.subscribe("/voice/castlex_xiaogu_tts_topic", 1, ttsCallback);	//	订阅语音合成话题

	ros::NodeHandle nh("~");    //用于launch文件传递参数
	nh.param("appid", appid, std::string("appid = 604adfa0, work_dir = ."));    //从launch文件获取参数
	...
	ros::Publisher yolo_pub = n.advertise<std_msgs::Int32>("yolo_asr_topic", 1);	//	发布语音识别话题
	ros::Publisher arv_pub = n.advertise<std_msgs::String>("arv_asr_topic", 1);	//	发布语音识别话题
	...
	while(ros::ok())
	{
		if(awake_flag)
		{
			...
			if(asr_flag)
			{
				std_msgs::Int32 yolo_msg;
				std_msgs::String arv_msg;
				arv_msg.data = result;
				yolo_msg.data = 1;    //将asr返回文本写入消息，发布到topic上
				// sleep(2);
				arv_pub.publish(arv_msg);
