...
//MSP_REC_STATUS_COMPLETE = 5识别结束：识别器返回的状态，提醒用户及时开始\停止获取识别结果。
...
static void iat_cb(char *data, unsigned long len, void *user_para)
{
	...
	if(sr == NULL || sr->ep_stat >= MSP_EP_AFTER_SPEECH) //若检测到后端点VAD，则跳出程序
		return;
	if (sr->state < SR_STATE_STARTED)
		return; /* 如果发生error/vad，则忽略数据 */
	
	//思考：这里是不是可以加上条件选择？当检测到前端点VAD才开始进行写入录音数据
	//但是如果需要写入才能检测到VAD，则说明前面的思考不可行
...
/* 如果aud_src不是SR_MIC，则将忽略devid; 使用 get_default_dev_id
 * 使用默认输入设备. 目前还没有提供设备列表功能. 
 */
...
int sr_write_audio_data(struct speech_rec *sr, char *data, unsigned int len)
{
	...
	if (MSP_EP_AFTER_SPEECH == sr->ep_stat) //检测到语音的后端点
		end_sr_on_vad(sr);
