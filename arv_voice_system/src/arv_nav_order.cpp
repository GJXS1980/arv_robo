...
string temp_path;
string pkg_path = ros::package::getPath("arv_voice_system");
string fo_("fo|");
...
const char* get_audio_file(void)
{
	...
	while(key != 27) //按Esc则退出
	{
		...
		case 1:
			printf("\n1.去物料台/中转台\n");
			return "/mnt/cgshare/zzt.wav";
		case 2:
			printf("\n2.回原点\n");
			return "/mnt/cgshare/yd.wav";
		case 3:
			printf("\n3.去立体仓库\n");
			return "/mnt/cgshare/ltck.wav";
...
int iatalk()
{
	// const char *login_config    = "appid = 604adfa0"; //登录参数
	UserData asr_data; 
	int ret                     = 0 ;
	char c;
...
int main(int argc, char* argv[])
{
	int ret = MSP_SUCCESS;
	printf(path1.data());
	ros::init(argc, argv, "arv_nav_order_node");    //初始化节点，向节点管理器注册
	ros::NodeHandle n;
	ros::Subscriber sub = n.subscribe("arv_awake_topic", 1, orderCallback);

	ros::NodeHandle nh("~");    //用于launch文件传递参数

	ros::Publisher pub = n.advertise<std_msgs::String>("arv_ocw_topic", 3);	// 发布离线命令词识别结果话题
	ros::Publisher cmd_pub = n.advertise<std_msgs::Int32>("arv_cmd_topic", 1);	//	识别离线命令词成功的flag话题

	ros::Rate loop_rate(10);    //10Hz循环周期

	nh.param("appid_param", appid_param, std::string("appid = 604adfa0, work_dir = ."));    //从launch文件获取参数
	...
	while(ros::ok())
	{
	const char *lgi_param = appid_param.data();
